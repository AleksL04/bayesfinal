...
$$
\mu_i = \alpha + \beta_1 (\text{log_st_mass}) + \beta_2 (\text{st_teff_k}) + \beta_3 (\text{st_met}) + \beta_4 (\text{sy_pnum})
$$
...
```{r}
posterior_vs_prior(fit_stanarm, pars = c("log_st_mass", "sy_pnum", "st_teff_k", "st_met")) +
...
```{r}
# Residuals vs Specific Predictors (Linearity Check)
clean_data_plot <- clean_data %>%
  mutate(residuals = residuals) # Use the residuals calculated in your previous chunk

# Plot against Mass
p1 <- ggplot(clean_data_plot, aes(x = log_st_mass, y = residuals)) +
  geom_point() + geom_smooth(method = "loess", se = FALSE, color = "blue") +
  geom_hline(yintercept = 0, linetype = "dashed") +
  ggtitle("Residuals vs Stellar Mass")

# Plot against Temperature
p2 <- ggplot(clean_data_plot, aes(x = st_teff_k, y = residuals)) +
  geom_point() + geom_smooth(method = "loess", se = FALSE, color = "blue") +
  geom_hline(yintercept = 0, linetype = "dashed") +
  ggtitle("Residuals vs Temp")
